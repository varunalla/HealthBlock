...
app.post('/uploadFile', upload.single('file'), (req, res) => {
    ...
    try {
      ...
      encryptor.encryptFile(req.file.path, fileName+'-'+key +'.dat', key, function(err) {
        console.log()
        const encryptedFile = fs.readFileSync('./'+fileName+'-'+key+'.dat');
        const params = {
        ...
        console.log(".dat file",fileName+'-'+key)
  ...
  app.post('/uploadTos3', (req, res) => {
    try {
      const params = {
        ...
        Key: req.body.key,
        Body: Buffer.from(req.body.file.data)
      };    
  ...
  app.post('/downloadFroms3', async (req, res) => {
    try {
      ...
      console.log("response of downloadFroms3",resp)
      res.status(200).json({ message: 'File downloaded successfully.', data: resp });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: 'An error occurred.' });
    }
  ...
  app.post('/downloadKeysFroms3', async (req, res) => {
    try {
      const params = {
        Bucket: req.body.bucket, 
        Key: req.body.key,
