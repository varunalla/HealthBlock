...
class WebObj {
  ...
  loadContract() {
    ...
    let tempContract = new this.web3.eth.Contract(
      this.contract.abi,
      healthBlockAddress
    );
...
const fetchUserProfile = async (userAddress, callback) => {
  try {
    let profile = await getWeb3Obj()
      .getHealthBlock()
      .methods.getPatientInfoAll(userAddress)
      .call();
...
const fetchDoctorProfile = async (userAddress, callback) => {
  try {
    let profile = await getWeb3Obj()
      .getHealthBlock()
      .methods.getDoctorInfoAll(userAddress)
      .call();
    callback(null, profile);
  } catch (e) {
    callback("error");
...
const fetchHCProviderProfile = async (userAddress, callback) => {
  try {
    let profile = await getWeb3Obj()
      .getHealthBlock()
      .methods.getHCProviderInfoAll(userAddress)
      .call();
    callback(null, profile);
  } catch (e) {
    console.log(e);
