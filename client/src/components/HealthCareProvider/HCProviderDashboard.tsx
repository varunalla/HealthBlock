...
const HCProviderDashboard: FunctionComponent<{}> = () => {
    ...
    const {handleApproveRequest, handleRejectRequest, verificationRequests, fetchRequests} = useContext(HealthContext)
    const logouthandler = () => {
        logout?.();
        ...
        <div className="mt-8 w-full max-w-screen-lg mx-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {verificationRequests && verificationRequests.map((request: { doctorName: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; credentialsHash: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; approved: any; }, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 text-center">{request.doctorName}</td>
                  <td className="px-6 py-4 text-center">{request.credentialsHash}</td>
                  <td className="px-6 py-4 text-center">{request.approved ? "Approved" : "Pending"}</td>
                  <td className="px-6 py-4 text-center">
                    {!request.approved && (
                      <>
                        <button className="px-3 py-1 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={() => approveRequest(index)}>Approve</button>
                        <button className="ml-2 px-3 py-1 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" onClick={() => rejectRequest(index)}>Reject</button>
                      </>
                    )}
                  </td>
                </tr>
